...
CREATE TABLE IF NOT EXISTS `farm_member` (
  ...
  `f_email` VARCHAR(40) NOT NULL COMMENT '농가 이메일',
  `f_phonenum` char(11) NULL COMMENT '농가 휴대폰번호', -- 휴대폰 인증 하게되면 not null 추가
  ...
  `f_profile_img` VARCHAR(200) NULL COMMENT '농가 프로필(사진)',
  `f_img` VARCHAR(200) NULL COMMENT '농가 (사진)',
  `f_bank` VARCHAR(10) NULL COMMENT '농가 입금은행',
  `f_bank_name` VARCHAR(200) NULL COMMENT '농가 입금자명',
  `f_bank_num` BIGINT NULL COMMENT '농가 계좌번호',
  `f_bank_img` VARCHAR(200) NULL COMMENT '농가 통장사본(사진)',
  `authority ` VARCHAR(10) NULL DEFAULT 'ROLE_USER' COMMENT '사용자 권한',
  `token ` VARCHAR(256) NULL COMMENT '토큰, 중복 로그인 확인용',
  PRIMARY KEY (`farm_id`),
  UNIQUE INDEX `f_email_UNIQUE` (`f_email` ASC) VISIBLE)
ENGINE = InnoDB;



insert into farm_member values (5,'123123', 'wkdch98@naver.com', '01011112222', '이원범', '9604041360000', '1234567891234', 12345, '충청북도 제천시', '원범농가', '01011113333', '안뇽원범농가', '사과, 수박', '프사1122', '농가사진1122', '신한', '신한원범', 110431111111, '통장사본1122', '', '')
...
CREATE TABLE IF NOT EXISTS `consumer_member` (
  ...
  `c_email` VARCHAR(40) NOT NULL COMMENT '소비자 이메일',
...
CREATE TABLE IF NOT EXISTS `grade` (
  ...
  UNIQUE INDEX `등급번호_UNIQUE` (`grade_id` ASC) VISIBLE)
ENGINE = InnoDB;
...
alter table consumer_member modify c_phonenum char (11);
